-- 画像URL永続化の修正
--
-- 問題: 20260317_sync_app_data_time_records.sql で sync_app_data 関数を再定義した際、
-- 20260313_add_image_url_packaging_equipment.sql で追加されていた packaging_items の
-- image_url 処理がリグレッション（脱落）していた。
-- また、image_url カラム追加のマイグレーションが未適用の可能性があるため、
-- ALTER TABLE を再実行する（IF NOT EXISTS で冪等性を担保）。
--
-- 修正内容:
-- 1. 4テーブルに image_url カラムを追加（未適用の場合のみ）
-- 2. sync_app_data 関数を再定義し、packaging_items の image_url を復元

-- テーブルに image_url カラムを追加（冪等）
ALTER TABLE materials ADD COLUMN IF NOT EXISTS image_url TEXT;
ALTER TABLE products ADD COLUMN IF NOT EXISTS image_url TEXT;
ALTER TABLE packaging_items ADD COLUMN IF NOT EXISTS image_url TEXT;
ALTER TABLE equipments ADD COLUMN IF NOT EXISTS image_url TEXT;

-- sync_app_data 関数を再定義（packaging_items の image_url リグレッション修正）
...
BEGIN
  ...
  INSERT INTO tmp_packaging_items (id, name, unit, size_description, currency, unit_cost, note, units_per_batch, image_url)
  SELECT COALESCE((value->>'id')::uuid, gen_random_uuid()),
         value->>'name',
         value->>'unit',
         value->>'size_description',
  ...
  INSERT INTO packaging_items (id, user_id, name, unit, size_description, currency, unit_cost, note, units_per_batch, image_url)
  SELECT tmp.id, p_user_id, tmp.name, tmp.unit, tmp.size_description, tmp.currency, tmp.unit_cost, tmp.note, tmp.units_per_batch, tmp.image_url
