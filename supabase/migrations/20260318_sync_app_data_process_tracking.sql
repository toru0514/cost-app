-- sync_app_data 関数に process_templates / product_processes の upsert/delete 処理を追加
-- time_records に product_id / product_process_id カラムのサポートを追加
...
BEGIN
  ...
  INSERT INTO tmp_products (
    id, name, category_large_id, category_medium_id, category_small_id,
    size_variants, base_man_hours, default_electricity_cost, sale_price,
    registered_at, notes, production_lot_size, expected_production_period_years,
    expected_production_quantity, equipment_ids, image_url, status
  )
  SELECT COALESCE((value->>'id')::uuid, gen_random_uuid()),
         value->>'name',
  ...
  INSERT INTO products (
    id, user_id, name, category_large_id, category_medium_id, category_small_id,
    size_variants, base_man_hours, default_electricity_cost, sale_price,
    registered_at, notes, production_lot_size, expected_production_period_years,
    expected_production_quantity, equipment_ids, image_url, status
  )
  SELECT tmp.id, p_user_id, tmp.name, tmp.category_large_id, tmp.category_medium_id, tmp.category_small_id,
         tmp.size_variants, tmp.base_man_hours, tmp.default_electricity_cost, tmp.sale_price,
         tmp.registered_at, tmp.notes, tmp.production_lot_size, tmp.expected_production_period_years,
         tmp.expected_production_quantity, tmp.equipment_ids, tmp.image_url, tmp.status
  ...
  -- cost entries (materials, packaging, labor, outsourcing, development, equipment, logistics, electricity, fees)
  -- ... existing cost entry handling unchanged ...

  v_stage := 'cost_entries_materials';
  CREATE TEMPORARY TABLE tmp_cost_entries_materials (id uuid, product_id uuid, material_id uuid, description text, usage_ratio numeric, cost_per_unit numeric, currency text) ON COMMIT DROP;
  INSERT INTO tmp_cost_entries_materials SELECT COALESCE((value->>'id')::uuid, gen_random_uuid()), nullif(value->>'product_id','')::uuid, nullif(value->>'material_id','')::uuid, nullif(value->>'description',''), nullif(value->>'usage_ratio','')::numeric, COALESCE(nullif(value->>'cost_per_unit','')::numeric, 0), value->>'currency' FROM jsonb_array_elements(COALESCE(p_payload->'cost_entries_materials', '[]'::jsonb)) AS value;
  INSERT INTO cost_entries_materials (id, user_id, product_id, material_id, description, usage_ratio, cost_per_unit, currency) SELECT tmp.id, p_user_id, tmp.product_id, tmp.material_id, tmp.description, tmp.usage_ratio, tmp.cost_per_unit, tmp.currency FROM tmp_cost_entries_materials tmp ON CONFLICT (user_id, id) DO UPDATE SET product_id = excluded.product_id, material_id = excluded.material_id, description = excluded.description, usage_ratio = excluded.usage_ratio, cost_per_unit = excluded.cost_per_unit, currency = excluded.currency;
  CREATE TEMPORARY TABLE tmp_deleted_cost_entries_materials (id uuid) ON COMMIT DROP;
  INSERT INTO tmp_deleted_cost_entries_materials SELECT (value->>'id')::uuid FROM jsonb_array_elements(COALESCE(p_payload->'cost_entries_materials_deleted', '[]'::jsonb)) AS value WHERE value ? 'id';
  ...
  CREATE TEMPORARY TABLE tmp_cost_entries_packaging (id uuid, product_id uuid, packaging_item_id uuid, quantity numeric, cost_per_unit numeric, currency text) ON COMMIT DROP;
  INSERT INTO tmp_cost_entries_packaging SELECT COALESCE((value->>'id')::uuid, gen_random_uuid()), nullif(value->>'product_id','')::uuid, nullif(value->>'packaging_item_id','')::uuid, COALESCE(nullif(value->>'quantity','')::numeric, 0), COALESCE(nullif(value->>'cost_per_unit','')::numeric, 0), value->>'currency' FROM jsonb_array_elements(COALESCE(p_payload->'cost_entries_packaging', '[]'::jsonb)) AS value;
  INSERT INTO cost_entries_packaging (id, user_id, product_id, packaging_item_id, quantity, cost_per_unit, currency) SELECT tmp.id, p_user_id, tmp.product_id, tmp.packaging_item_id, tmp.quantity, tmp.cost_per_unit, tmp.currency FROM tmp_cost_entries_packaging tmp ON CONFLICT (user_id, id) DO UPDATE SET product_id = excluded.product_id, packaging_item_id = excluded.packaging_item_id, quantity = excluded.quantity, cost_per_unit = excluded.cost_per_unit, currency = excluded.currency;
  CREATE TEMPORARY TABLE tmp_deleted_cost_entries_packaging (id uuid) ON COMMIT DROP;
  INSERT INTO tmp_deleted_cost_entries_packaging SELECT (value->>'id')::uuid FROM jsonb_array_elements(COALESCE(p_payload->'cost_entries_packaging_deleted', '[]'::jsonb)) AS value WHERE value ? 'id';
  ...
  CREATE TEMPORARY TABLE tmp_cost_entries_labor (id uuid, product_id uuid, labor_role_id uuid, hours numeric, people_count numeric, hourly_rate_override numeric) ON COMMIT DROP;
  INSERT INTO tmp_cost_entries_labor SELECT COALESCE((value->>'id')::uuid, gen_random_uuid()), nullif(value->>'product_id','')::uuid, nullif(value->>'labor_role_id','')::uuid, COALESCE(nullif(value->>'hours','')::numeric, 0), COALESCE(nullif(value->>'people_count','')::numeric, 0), nullif(value->>'hourly_rate_override','')::numeric FROM jsonb_array_elements(COALESCE(p_payload->'cost_entries_labor', '[]'::jsonb)) AS value;
  INSERT INTO cost_entries_labor (id, user_id, product_id, labor_role_id, hours, people_count, hourly_rate_override) SELECT tmp.id, p_user_id, tmp.product_id, tmp.labor_role_id, tmp.hours, tmp.people_count, tmp.hourly_rate_override FROM tmp_cost_entries_labor tmp ON CONFLICT (user_id, id) DO UPDATE SET product_id = excluded.product_id, labor_role_id = excluded.labor_role_id, hours = excluded.hours, people_count = excluded.people_count, hourly_rate_override = excluded.hourly_rate_override;
  CREATE TEMPORARY TABLE tmp_deleted_cost_entries_labor (id uuid) ON COMMIT DROP;
  INSERT INTO tmp_deleted_cost_entries_labor SELECT (value->>'id')::uuid FROM jsonb_array_elements(COALESCE(p_payload->'cost_entries_labor_deleted', '[]'::jsonb)) AS value WHERE value ? 'id';
  ...
  CREATE TEMPORARY TABLE tmp_cost_entries_outsourcing (id uuid, product_id uuid, cost_per_unit numeric, currency text, note text) ON COMMIT DROP;
  INSERT INTO tmp_cost_entries_outsourcing SELECT COALESCE((value->>'id')::uuid, gen_random_uuid()), nullif(value->>'product_id','')::uuid, COALESCE(nullif(value->>'cost_per_unit','')::numeric, 0), value->>'currency', nullif(value->>'note','') FROM jsonb_array_elements(COALESCE(p_payload->'cost_entries_outsourcing', '[]'::jsonb)) AS value;
  INSERT INTO cost_entries_outsourcing (id, user_id, product_id, cost_per_unit, currency, note) SELECT tmp.id, p_user_id, tmp.product_id, tmp.cost_per_unit, tmp.currency, tmp.note FROM tmp_cost_entries_outsourcing tmp ON CONFLICT (user_id, id) DO UPDATE SET product_id = excluded.product_id, cost_per_unit = excluded.cost_per_unit, currency = excluded.currency, note = excluded.note;
  CREATE TEMPORARY TABLE tmp_deleted_cost_entries_outsourcing (id uuid) ON COMMIT DROP;
  INSERT INTO tmp_deleted_cost_entries_outsourcing SELECT (value->>'id')::uuid FROM jsonb_array_elements(COALESCE(p_payload->'cost_entries_outsourcing_deleted', '[]'::jsonb)) AS value WHERE value ? 'id';
  ...
  CREATE TEMPORARY TABLE tmp_cost_entries_development (id uuid, product_id uuid, title text, prototype_labor_cost numeric, prototype_material_cost numeric, tooling_cost numeric, amortization_years int) ON COMMIT DROP;
  INSERT INTO tmp_cost_entries_development SELECT COALESCE((value->>'id')::uuid, gen_random_uuid()), nullif(value->>'product_id','')::uuid, nullif(value->>'title',''), COALESCE(nullif(value->>'prototype_labor_cost','')::numeric, 0), COALESCE(nullif(value->>'prototype_material_cost','')::numeric, 0), COALESCE(nullif(value->>'tooling_cost','')::numeric, 0), COALESCE(nullif(value->>'amortization_years','')::int, 1) FROM jsonb_array_elements(COALESCE(p_payload->'cost_entries_development', '[]'::jsonb)) AS value;
  INSERT INTO cost_entries_development (id, user_id, product_id, title, prototype_labor_cost, prototype_material_cost, tooling_cost, amortization_years) SELECT tmp.id, p_user_id, tmp.product_id, tmp.title, tmp.prototype_labor_cost, tmp.prototype_material_cost, tmp.tooling_cost, tmp.amortization_years FROM tmp_cost_entries_development tmp ON CONFLICT (user_id, id) DO UPDATE SET product_id = excluded.product_id, title = excluded.title, prototype_labor_cost = excluded.prototype_labor_cost, prototype_material_cost = excluded.prototype_material_cost, tooling_cost = excluded.tooling_cost, amortization_years = excluded.amortization_years;
  CREATE TEMPORARY TABLE tmp_deleted_cost_entries_development (id uuid) ON COMMIT DROP;
  INSERT INTO tmp_deleted_cost_entries_development SELECT (value->>'id')::uuid FROM jsonb_array_elements(COALESCE(p_payload->'cost_entries_development_deleted', '[]'::jsonb)) AS value WHERE value ? 'id';
  ...
  CREATE TEMPORARY TABLE tmp_cost_entries_equipment (id uuid, product_id uuid, equipment_id uuid, allocation_ratio numeric, annual_quantity numeric, usage_hours numeric) ON COMMIT DROP;
  INSERT INTO tmp_cost_entries_equipment SELECT COALESCE((value->>'id')::uuid, gen_random_uuid()), nullif(value->>'product_id','')::uuid, nullif(value->>'equipment_id','')::uuid, COALESCE(nullif(value->>'allocation_ratio','')::numeric, 0), COALESCE(nullif(value->>'annual_quantity','')::numeric, 0), nullif(value->>'usage_hours','')::numeric FROM jsonb_array_elements(COALESCE(p_payload->'cost_entries_equipment', '[]'::jsonb)) AS value;
  INSERT INTO cost_entries_equipment (id, user_id, product_id, equipment_id, allocation_ratio, annual_quantity, usage_hours) SELECT tmp.id, p_user_id, tmp.product_id, tmp.equipment_id, tmp.allocation_ratio, tmp.annual_quantity, tmp.usage_hours FROM tmp_cost_entries_equipment tmp ON CONFLICT (user_id, id) DO UPDATE SET product_id = excluded.product_id, equipment_id = excluded.equipment_id, allocation_ratio = excluded.allocation_ratio, annual_quantity = excluded.annual_quantity, usage_hours = excluded.usage_hours;
  CREATE TEMPORARY TABLE tmp_deleted_cost_entries_equipment (id uuid) ON COMMIT DROP;
  INSERT INTO tmp_deleted_cost_entries_equipment SELECT (value->>'id')::uuid FROM jsonb_array_elements(COALESCE(p_payload->'cost_entries_equipment_deleted', '[]'::jsonb)) AS value WHERE value ? 'id';
  ...
  CREATE TEMPORARY TABLE tmp_cost_entries_logistics (id uuid, product_id uuid, shipping_method_id uuid, cost_per_unit numeric, currency text) ON COMMIT DROP;
  INSERT INTO tmp_cost_entries_logistics SELECT COALESCE((value->>'id')::uuid, gen_random_uuid()), nullif(value->>'product_id','')::uuid, nullif(value->>'shipping_method_id','')::uuid, COALESCE(nullif(value->>'cost_per_unit','')::numeric, 0), value->>'currency' FROM jsonb_array_elements(COALESCE(p_payload->'cost_entries_logistics', '[]'::jsonb)) AS value;
  INSERT INTO cost_entries_logistics (id, user_id, product_id, shipping_method_id, cost_per_unit, currency) SELECT tmp.id, p_user_id, tmp.product_id, tmp.shipping_method_id, tmp.cost_per_unit, tmp.currency FROM tmp_cost_entries_logistics tmp ON CONFLICT (user_id, id) DO UPDATE SET product_id = excluded.product_id, shipping_method_id = excluded.shipping_method_id, cost_per_unit = excluded.cost_per_unit, currency = excluded.currency;
  CREATE TEMPORARY TABLE tmp_deleted_cost_entries_logistics (id uuid) ON COMMIT DROP;
  INSERT INTO tmp_deleted_cost_entries_logistics SELECT (value->>'id')::uuid FROM jsonb_array_elements(COALESCE(p_payload->'cost_entries_logistics_deleted', '[]'::jsonb)) AS value WHERE value ? 'id';
  ...
  CREATE TEMPORARY TABLE tmp_cost_entries_electricity (id uuid, product_id uuid, cost_per_unit numeric, currency text) ON COMMIT DROP;
  INSERT INTO tmp_cost_entries_electricity SELECT COALESCE((value->>'id')::uuid, gen_random_uuid()), nullif(value->>'product_id','')::uuid, COALESCE(nullif(value->>'cost_per_unit','')::numeric, 0), value->>'currency' FROM jsonb_array_elements(COALESCE(p_payload->'cost_entries_electricity', '[]'::jsonb)) AS value;
  INSERT INTO cost_entries_electricity (id, user_id, product_id, cost_per_unit, currency) SELECT tmp.id, p_user_id, tmp.product_id, tmp.cost_per_unit, tmp.currency FROM tmp_cost_entries_electricity tmp ON CONFLICT (user_id, id) DO UPDATE SET product_id = excluded.product_id, cost_per_unit = excluded.cost_per_unit, currency = excluded.currency;
  CREATE TEMPORARY TABLE tmp_deleted_cost_entries_electricity (id uuid) ON COMMIT DROP;
  INSERT INTO tmp_deleted_cost_entries_electricity SELECT (value->>'id')::uuid FROM jsonb_array_elements(COALESCE(p_payload->'cost_entries_electricity_deleted', '[]'::jsonb)) AS value WHERE value ? 'id';
  ...
  CREATE TEMPORARY TABLE tmp_cost_entries_fees (id uuid, product_id uuid, fee_id uuid, rate_percent numeric, fixed_amount numeric, currency text) ON COMMIT DROP;
  INSERT INTO tmp_cost_entries_fees SELECT COALESCE((value->>'id')::uuid, gen_random_uuid()), nullif(value->>'product_id','')::uuid, nullif(value->>'fee_id','')::uuid, COALESCE(nullif(value->>'rate_percent','')::numeric, 0), COALESCE(nullif(value->>'fixed_amount','')::numeric, 0), value->>'currency' FROM jsonb_array_elements(COALESCE(p_payload->'cost_entries_fees', '[]'::jsonb)) AS value;
  INSERT INTO cost_entries_fees (id, user_id, product_id, fee_id, rate_percent, fixed_amount, currency) SELECT tmp.id, p_user_id, tmp.product_id, tmp.fee_id, tmp.rate_percent, tmp.fixed_amount, tmp.currency FROM tmp_cost_entries_fees tmp ON CONFLICT (user_id, id) DO UPDATE SET product_id = excluded.product_id, fee_id = excluded.fee_id, rate_percent = excluded.rate_percent, fixed_amount = excluded.fixed_amount, currency = excluded.currency;
  CREATE TEMPORARY TABLE tmp_deleted_cost_entries_fees (id uuid) ON COMMIT DROP;
  INSERT INTO tmp_deleted_cost_entries_fees SELECT (value->>'id')::uuid FROM jsonb_array_elements(COALESCE(p_payload->'cost_entries_fees_deleted', '[]'::jsonb)) AS value WHERE value ? 'id';
  ...
  -- process_templates の upsert/delete 処理（products より後に実行: FK制約あり）
  v_stage := 'process_templates';
  CREATE TEMPORARY TABLE tmp_process_templates (
    id uuid,
    parent_id uuid,
    name text,
  ...
  INSERT INTO tmp_process_templates (id, parent_id, name, default_hourly_rate, color, icon, sort_order)
  SELECT COALESCE((value->>'id')::uuid, gen_random_uuid()),
         nullif(value->>'parent_id','')::uuid,
         COALESCE(value->>'name', ''),
         COALESCE(nullif(value->>'default_hourly_rate','')::numeric, 0),
         nullif(value->>'color',''),
         nullif(value->>'icon',''),
         COALESCE(nullif(value->>'sort_order','')::integer, 0)
  FROM jsonb_array_elements(COALESCE(p_payload->'process_templates', '[]'::jsonb)) AS value;
  INSERT INTO process_templates (id, user_id, parent_id, name, default_hourly_rate, color, icon, sort_order)
  SELECT tmp.id, p_user_id, tmp.parent_id, tmp.name, tmp.default_hourly_rate, tmp.color, tmp.icon, tmp.sort_order
  FROM tmp_process_templates tmp
  ON CONFLICT (user_id, id) DO UPDATE SET
  ...
  CREATE TEMPORARY TABLE tmp_deleted_process_templates (id uuid) ON COMMIT DROP;
  INSERT INTO tmp_deleted_process_templates (id)
  SELECT (value->>'id')::uuid
  FROM jsonb_array_elements(COALESCE(p_payload->'process_templates_deleted', '[]'::jsonb)) AS value
  WHERE value ? 'id';
  DELETE FROM process_templates WHERE user_id::text = p_user_id::text AND id::text IN (SELECT id::text FROM tmp_deleted_process_templates);

  -- product_processes の upsert/delete 処理
  v_stage := 'product_processes';
  CREATE TEMPORARY TABLE tmp_product_processes (
    id uuid,
    product_id uuid,
    parent_id uuid,
  ...
  INSERT INTO tmp_product_processes (id, product_id, parent_id, process_template_id, name, hourly_rate, estimated_minutes, sort_order)
  SELECT COALESCE((value->>'id')::uuid, gen_random_uuid()),
         nullif(value->>'product_id','')::uuid,
         nullif(value->>'parent_id','')::uuid,
         nullif(value->>'process_template_id','')::uuid,
         COALESCE(value->>'name', ''),
         COALESCE(nullif(value->>'hourly_rate','')::numeric, 0),
         nullif(value->>'estimated_minutes','')::numeric,
         COALESCE(nullif(value->>'sort_order','')::integer, 0)
  FROM jsonb_array_elements(COALESCE(p_payload->'product_processes', '[]'::jsonb)) AS value;
  INSERT INTO product_processes (id, user_id, product_id, parent_id, process_template_id, name, hourly_rate, estimated_minutes, sort_order)
  SELECT tmp.id, p_user_id, tmp.product_id, tmp.parent_id, tmp.process_template_id, tmp.name, tmp.hourly_rate, tmp.estimated_minutes, tmp.sort_order
  FROM tmp_product_processes tmp
  ON CONFLICT (user_id, id) DO UPDATE SET
  ...
  CREATE TEMPORARY TABLE tmp_deleted_product_processes (id uuid) ON COMMIT DROP;
  INSERT INTO tmp_deleted_product_processes (id)
  SELECT (value->>'id')::uuid
  FROM jsonb_array_elements(COALESCE(p_payload->'product_processes_deleted', '[]'::jsonb)) AS value
  WHERE value ? 'id';
  DELETE FROM product_processes WHERE user_id::text = p_user_id::text AND id::text IN (SELECT id::text FROM tmp_deleted_product_processes);

  -- time_records の upsert/delete 処理（product_id, product_process_id を含む）
  ...
  CREATE TEMPORARY TABLE tmp_time_records (
    ...
    note text,
    product_id uuid,
    product_process_id uuid,
    created_at timestamptz,
    updated_at timestamptz
  ) ON COMMIT DROP;
  INSERT INTO tmp_time_records (id, task_name, total_duration, laps, note, product_id, product_process_id, created_at, updated_at)
  SELECT COALESCE((value->>'id')::uuid, gen_random_uuid()),
         ...
         nullif(value->>'note',''),
         nullif(value->>'product_id','')::uuid,
         nullif(value->>'product_process_id','')::uuid,
  ...
  INSERT INTO time_records (id, user_id, task_name, total_duration, laps, note, product_id, product_process_id, created_at, updated_at)
  SELECT tmp.id, p_user_id, tmp.task_name, tmp.total_duration, tmp.laps, tmp.note, tmp.product_id, tmp.product_process_id, tmp.created_at, tmp.updated_at
  ...
  ON CONFLICT (user_id, id) DO UPDATE SET
    ...
    note = excluded.note,
    product_id = excluded.product_id,
    product_process_id = excluded.product_process_id,
