...
- [ ] 入力中に原価・粗利・利益率などのサマリをリアルタイム表示するサイドパネルを追加し、即時フィードバックを得られるようにする。
- [ ] 商品一覧・マスタ一覧に検索/フィルタ/ソート機能を追加し、目的のエントリへ素早くアクセスできるようにする。
- [ ] 集計タブの可視化に期間比較やカテゴリフィルタを導入し、トレンドを把握できるようにする。
- [ ] 操作ボタン（編集/コピー/削除やエクスポート）を行末でアイコン＋テキストにまとめ、主なアクションをヘッダーからアクセスできるショートカットも用意する。
