...
- [ ] マスタをスプレッドシートで管理し、Supabase への同期ボタンを実装して一括更新できるようにする。
  - [ ] **商品スプレッドシート連携（概要）**: テンプレート定義→CSV アップロード→検証→ステージング→本登録→履歴管理の流れを整備し、大量の登録/更新を安全に行えるようにする。
  - [ ] インポート用テンプレートを Google スプレッドシート/CSV で提供し、必須列（商品名・カテゴリ・販売価格など）と入力ルールを明文化する。
  - [ ] 商品タブに CSV インポート UI を追加し、アップロード後に行ごとのプレビューと入力値の確認を行えるようにする。
  - [ ] インポートデータ検証（必須列、数値型、カテゴリ名→ID 変換、重複チェック）とエラー表示/再編集フローを実装する。
  - [ ] 検証済みデータをステージングし、「新規/上書き/スキップ」を行単位で選べるようにした上で、Supabase へ一括登録するトランザクション処理を実装する。
  - [ ] 直近のインポート履歴（実行者・時刻・件数・失敗行）および CSV エクスポート機能を追加し、監査ログにも記録する。
