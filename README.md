...
- [x] 商品一覧データを CSV としてエクスポートできる機能を実装し、外部分析や共有を容易にする。
- [x] 商品一覧に配送方法・使用設備の列を追加し、CSV エクスポートにも反映する。
- [x] 設備導入シミュレーション機能を追加し、配賦額や想定生産数を変えた場合の投資回収を比較検討できるようにする。
- [x] 商品登録フォームで必須項目（商品名など）が空の場合にトーストで不足項目を案内し、登録を抑止する。
- [x] マスタ・商品登録フォームに「コピーを作成」ボタンを追加し、既存データを複製して編集を開始できるようにする。
- [x] 材料・梱包材入力で「複数個セット購入（例: 100枚で◯円）」を入力可能にし、1個あたり単価へ自動換算できるようにする。
- [x] 目標利益率シミュレーションを追加し、販売価格や数量から逆算できるようにする。
- [x] 設備シミュレーションの計算式を見直し、利用割合を踏まえた妥当な販売価格/粗利計算にする。
- [x] カテゴリ別ランキングやコスト構成グラフなどの可視化カードを追加する。
- [x] 商品登録・マスタ登録フォームをセクション単位の折りたたみ/ステップUIに再設計し、情報量を段階的に提示する。
- [x] 専門用語や入力項目にツールチップ/インラインヘルプを追加し、初めてのユーザーでも迷わないよう補助する。
- [x] 入力中に原価・粗利・利益率などのサマリをリアルタイム表示するサイドパネルを追加し、即時フィードバックを得られるようにする。
...
- [x] タブを跨いでも検索条件やフィルタが保持されるよう状態を保存し、シミュレーション条件をプリセット共有できる仕組みを追加する。
...
- [x] ログイン・権限対応後にスマホUI微調整（タブ保持やボトムナビ化など）を行い、最終モバイル最適化を完了する。
- [x] Supabase 保存処理をDELETE→INSERTの直列ではなくトランザクションやテンポラリテーブル経由に変更し、途中失敗でデータが空になるリスクを無くす。
- [x] 保存操作の監査ログを Supabase に記録し、「誰が・いつ・どの端末から保存したか」を確認できるようにする。
- [ ] マスタをスプレッドシートで管理し、Supabase への同期ボタンを実装して一括更新できるようにする。
  - [x] **商品スプレッドシート連携（概要）**: テンプレート定義→CSV アップロード→検証→ステージング→本登録→履歴管理の流れを整備し、大量の登録/更新を安全に行えるようにする。
  - [x] インポート用テンプレートを Google スプレッドシート/CSV で提供し、必須列（商品名・カテゴリ・販売価格など）と入力ルールを明文化する。
    - `templates/product-import-template.csv` にサンプル行入りのテンプレートを配置。Google スプレッドシートで読み込み、列構成を崩さずに上書き保存すればアプリ側のテンプレダウンロードボタンと互換。
    - 列仕様

      | 列キー | 表示名 | 必須 | 入力ルール |
      | --- | --- | --- | --- |
      | `product_name` | 商品名 | ✅ | 1 行に 1 商品。重複時は後段ステージングで上書き候補として扱う想定。 |
      | `category_large` / `category_medium` / `category_small` | カテゴリ | 任意 | 既存カテゴリ名と一致する文字列を入力。親子不整合は「要カテゴリ確認」扱い。 |
      | `sale_price` | 販売価格 | ✅ | 半角数字。カンマ可。0 以下はエラー。 |
      | `base_man_hours` | 工数 (時間) | ✅ | 小数可。0 未満はエラー。 |
      | `expected_period_years` | 想定期間(年) | 任意 | 未入力時は 1 年扱い。 |
      | `expected_quantity` | 想定生産数 | ✅ | 正の整数。 |
      | `size_variants` | サイズ/バリエーション | 任意 | JSON (`[{"label":"M","quantity":500}]`) か `S:500|M:300` 形式。 |
      | `notes` | 備考 | 任意 | 自由入力。 |

    - 運用フロー
      1. 上記テンプレを Google スプレッドシートへインポートし、商品情報を記入する。
      2. 「ファイル → ダウンロード → CSV」で書き出し、商品タブの「商品CSVインポート（β）」にアップロード。
      3. アップロード後のプレビューでエラー行は修正→再エクスポートし、問題ない行はステージング→Supabase 連携タスクへ進む。
  - [x] 商品タブに CSV インポート UI を追加し、アップロード後に行ごとのプレビューと入力値の確認を行えるようにする。
  - [x] インポートデータ検証（必須列、数値型、カテゴリ名→ID 変換、重複チェック）とエラー表示/再編集フローを実装する。
  ...
  - [x] 管理者専用 API ルート `/api/admin/sheet-settings` を追加し、サービスロールキー経由で任意ユーザーのシート設定を upsert できるようにする。
    ...
    - `SHEET_SETTINGS_ADMIN_EMAILS` に操作を許可するメールアドレスをカンマ区切りで登録。
    - `NEXT_PUBLIC_SHEET_SETTINGS_ADMIN_EMAILS` に同じメールアドレス一覧を設定し、クライアント側の UI 表示を制御する（管理者以外には「シート割り当て」フォームを表示しない）。
...
- **設備投資の扱い**
  ...
  - **シミュレーションカード**: Cost タブで設備ごとに年間数量・販売価格を入力すると、設備単価・年間粗利・回収率・回収年数を即時計算し、投資回収シナリオを比較できるようにする。
