...
人件費の役割（レート）と設備を同じセクションで管理します。

### 人件費レート

作業者の役割（職種）ごとの時間単価を登録します。
...
設備の配分コストを試算する機能です。設備の使用時間や配分方法を変更した場合の商品あたりのコスト変動を確認できます。
...
商品のサイズバリエーション（ラベルと数量のセット）をプリセットとして登録できます。商品登録時にプリセットを選択するだけで、バリエーションを一括設定できます。

### 登録手順

1. 「**オプションプリセット**」タブを選択します
2. プリセット名を入力します（例: 「標準3サイズ」）
3. バリエーションを追加します（ラベルと数量）
4. 保存します

### 活用方法

商品登録画面のステップ1で「オプションプリセット」のドロップダウンからプリセットを選択し、「適用」ボタンを押すとバリエーションが一括設定されます。
...
flowchart TD
    ...
    E --> F[5. 手数料を登録]
    F --> G[6. 人件費レート・設備を登録]
    G --> H[7. 必要に応じて工程テンプレートを登録]
    H --> I[8. 必要に応じてオプションプリセットを登録]
